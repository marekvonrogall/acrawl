...
from constants import FETCHING_DATE, Color, BASE_DIR, Frequency, DEFAULT_RESOLUTION
...
def fetch_data(delete_previous_data=True, get_daily_cme_movie_frames=True):
    ...
    for frequency in Frequency:
        download_data(get_latest_sun_image_url(DEFAULT_RESOLUTION.value, frequency.value, False))
        download_data(get_latest_sun_image_url(DEFAULT_RESOLUTION.value, frequency.value, True))
        download_data(get_latest48h_video_url(frequency=frequency.value))
    ...
    for arg in args:
        ...
        for data_item in items:
            ...
            if isinstance(data_item["url"], str):
                ...
                print(f"{Color.OKCYAN}[INFO]{Color.ENDC} Downloading: {data_item["source"]}: \"{data_item["name"]}.{data_item["format"]}\" ({data_item["url"]}) - {download_path}")
                try:
                ...
                print(f"{Color.OKCYAN}[INFO]{Color.ENDC}: Downloading: CME Movie Frames ({data_item['url']['page']})")
                date = data_item["date"]
